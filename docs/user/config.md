...
- **行为**:
  ...
  - ⚠️ **抽不出正文时会退回整个响应体做匹配**：流式格式认不出、或流里根本没有文本产出时，
    关键字是拿**整包原始响应体**（含协议元数据）来匹配的。所以关键字要挑**只可能出现在模型
    回答里**的串——像 `"ok"`、`"content"` 这种在协议字段名里也会出现的词，可能在模型一个字
    都没输出时照样判绿。内置 arith 模板用 `RP_ANSWER=<和>` 正是为此（题面与元数据里都不会出现）。
  - 探测判红时，`content_mismatch` 会在管理后台的「错误摘要」里给出结构化诊断：期望的关键字、
    实际抽到多少字符、SSE 事件数与终止事件、上游自报的错误与截断原因。其中
    `matched_against=raw_body` 就表示触发了上面那条整包回退。
