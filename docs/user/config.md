...
>
> 1. **验证名单真的进了运行态**。首次启用时若加载失败，热更新只会保留「尚无名单」的旧配置——
>    这对安全目标而言**不是** fail-closed。查启动/热更新日志里 `API Key 认证索引已重建` 那行的
>    `revoked_keys=` 是否等于预期条目数。
> 2. **人工处置存量的 pending/approved 变更请求**。admin 侧对「认证 key」的追溯依赖
>    「名单 ∩ 当前配置在用 key」推导 HMAC 指纹；若某把泄露 key 已被轮换出配置，用它认证过的
>    历史请求就推导不出、会漏检（请求携带的 `new_api_key` 因密文可解则是精确判定，不受此限）。
>    所以名单上线时应把此前的待审队列过一遍，该驳回的驳回。
