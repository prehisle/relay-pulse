...
- **聚合口径**: 上式对整个时间窗口成立——所有探测进同一个分母，**不是**先算每个时间块的可用率再等权平均
  （那会让只跑了几次的时间块和跑满的一整天同权）。多模型通道下，该通道所有模型的探测一并计入
...
- **类型**: bool
- **默认值**: `false`
- **说明**: 隐藏列表顶部的「模型厂商」筛选器。顶部工具栏的宽度预算是死的（容器 max-width 1216px），加入「模型」筛选器后厂商这一格往往放不下；且厂商只有个位数取值、绝大多数通道集中在两三家，筛选力度本就弱于按模型筛。
- **注意**: 隐藏的只是筛选入口，**厂商列不受影响**（列是观察维度，筛选器是操作轴）。开启后 URL 里残留的 `?vendor=` 会被视为未选中、不再过滤数据（参数本身保留，关掉开关即恢复原选择）。
...
- **类型**: string
- **默认值**: `""`（空）
- **说明**: 移入冷板的原因说明（仅用于 `board: cold` 的监测项）。内容会**公开显示**在状态页：冷板通道在列表、移动端卡片、网格卡片三处都常驻一行 ❄ 原因（过长截断，完整内容走悬浮提示）；桌面端通道名的 hover 提示里另有一份完整文案
- **配置方式**: 直接写 yaml，或在管理后台的通道详情里把板块选成「冷板」后填写（该输入框仅在冷板下出现）
- **约束**: 仅当 `board: cold` 时有效；如果在非冷板项中配置，启动时会输出警告并自动清空（仅内存）。经管理后台保存时还会连磁盘上的值一并清掉，避免「改回热板 → 再设冷板」时复活一条过期原因
...
可用率按**通道级**计算：同一 PSC 下所有活跃监测行（父通道 + 各子模型，排除 disabled/hidden/cold）
的探测记录合并成一个分母，与前端展示的可用率同口径。
...
| `min_probes` | int | `10` | 7 天内**每个模型行**平均最少探测次数，不足则跳过判断（保护新通道）。多模型通道的实际门槛是 `min_probes × 活跃行数` |
...
- 组级状态取所有层的最差状态（红 > 黄 > 绿）
- 组级可用率把**所有层的探测汇总进同一个分母**（可用探测数 / 总探测数），不是取各层最小值——
  一个模型故障不会吃掉同通道其余模型的成绩。自动移板的 7 天可用率同口径
...
- **行为**:
  ...
  - **流式响应只拿模型正文来匹配**：流里没有文本产出时，匹配对象就是空的（判红），
    **不会**退回整包响应体。尤其是 OpenAI Responses 协议的**思考摘要**
    （`reasoning_summary_text.*`）不算正文——模型只在思考里写出答案、正文一个字没输出，
    判红而不是判绿。
  - ⚠️ **非流式响应仍是整体匹配**：那时响应体本身就是模型输出（或上游的错误信息），
    所以关键字要挑**只可能出现在模型回答里**的串——像 `"ok"`、`"content"` 这种在协议
    字段名里也会出现的词，可能在模型一个字都没输出时照样判绿。内置 arith 模板用
    `RP_ANSWER=<和>` 正是为此（题面与元数据里都不会出现）。
  - 探测判红时，`content_mismatch` 会在管理后台的「错误摘要」里给出结构化诊断：期望的关键字、
    实际抽到多少字符、SSE 事件数与终止事件、上游自报的错误与截断原因。其中
    `matched_against=none` 表示这条流里根本没有模型正文——是上游没产出，不是模型答错了。
