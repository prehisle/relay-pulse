...
- **自助收录已开放第一方厂商模型**（v2.80+）：`SubmitRequest`/`Submission` 带行级 `model`+`model_vendor`，唯一判定是 `onboarding.ValidateModelSelection`（native 漏填即拒、非 native 多填即拒、model 走严格 ASCII 白名单——body 是原始字节且 `{{MODEL}}` 是纯字符串替换，一个双引号就打烂 JSON）。四个入口全接同一条：`/onboarding/test`、`/onboarding/submit`、`AdminUpdate`（model 支持**显式清空**）、`AdminPublish` 的 `validateMonitorConfig`（位于 `AdminConfigJSON` 整份覆盖之后）。厂商由**用户自己填**（站长 2026-08-21 拍板，推翻旧策略：撒谎杀伤面有界 + 必过 admin 审核 + rpdiag 盲评兜底）。
