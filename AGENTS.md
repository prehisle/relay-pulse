...
- **cx 侧两套形态并存（v2.92.0 起），新建 cx 模板前先决定照哪套**：**订阅态**＝`cx-gpt-arith` + `cxGPTWireDerivedTemplates` 登记的成员（真 codex CLI 身份头、版本见 `cx-gpt-arith` 的 `_comment`、**无** `openai-beta`、body 带 `prompt_cache_key`）；**平台态**＝`cx-gpt54-arith` + `cx-native-*` 族（假 UA、带 `openai-beta`）。订阅态族全员的 `headers`/`body` **逐字节相同**、形态依据只写在 `cx-gpt-arith` 的 `_comment`（族内单一真相源），改动必须全族同步——改一份漏其余**不报错**，只会让族内模型跑在两套指纹上。守卫 `TestCxGPTSubscriptionFamilyShareWireShape` / `TestCxGPTFamilyMembersAreAllClassified`（每个 `cx-gpt*.json` 必须显式归类）。⚠️ 转正依据是「**不会变差**」不是「有收益」（73 端点 A/B，通过率 p=0.852、延迟 p=0.698，无独有失败类别）——**别再提「换形态救红通道」**。
