...
**非默认约定与硬契约：**
1. **自动移板**: `automove.Service` 基于 7 天可用率**与 rpdiag 质量信号**移板。**可用率是通道级的**：override 仍只落在根行，但历史查询展开同 PSC 下全部活跃行（父+子模型，排除 disabled/hidden/配置 cold），所有探测进同一个分母（`historyByPSC` + `CalculateAvailability`），与前端展示口径一致——2026-09-10 前它只看根行，同一条通道能出现「面板 54.92% / 移板按 78.79% 判」这种 24pp 分裂。连带语义：`min_probes` 是**每个活跃行**的平均样本数，实际门槛为 `min_probes × 活跃行数`，否则多模型通道白拿 N 倍宽松。配置板位（board）是"锚点/天花板"——只在配置板位及以下浮动、绝不向上越板（board=secondary 不会被自动升 hot；board=hot 可降 secondary 再恢复）。cold 为 sticky，需 `auto_cold_exempt` 手动解除。**双 latch 分离**（v2.65.0）：可用率迟滞与质量 latch 各自独立记忆，合成板位 `sticky-cold/可用率cold > (可用率secondary 或 质量latch) > 配置hot`——某通道任一**活跃** rpdiag 评测模型近3次全 hard-fail（`recent_attempts` 尾3全 null + `hard_fail_active`）→ 质量 latch 封顶 secondary（只封 secondary、绝不推 cold），恢复后连续 `qualityRecoveryDebounce=2` 个新鲜快照升回；feed 拉不到/过 TTL/未接 rpdiag → 冻结现状不动。质量优先，赞助/置顶同样被质量降板（合同例外靠给通道挂 `auto_move_exempt`——同一个 flag 整体豁免可用率+质量）。移板原因 `board_reason`（机器码 `quality_hardfail`）+ `board_reason_models`（触发 model 名）经 `/api/status` 扁平+分组下发前端。**展示两处**：① 通道名 hover tooltip（四语言，`components/table/ChannelCell`）；② **标注列常驻 negative 徽章**（v2.66+，`config.deriveSystemAnnotations` 读运行时注入的 `ServiceConfig.BoardReason=="quality_hardfail"` 派生 id=`quality_hardfail`/priority=5 排风险之后，前端 `AnnotationChip` 按 id 路由 `QualityDemoteIcon`；后端直出中文 label/tooltip）——一眼可见无需 hover。注意质量移板徽章是 negative 家族，会命中 `sortMonitors.meetsPinCriteria` 的「negative→不置顶」既有逻辑，即质量移板通道失去赞助置顶资格（符合「质量优先，赞助/置顶也降」）。注解重算在 `automove.applyOverrideToMonitor`：board 或 sponsor 任一覆盖即用覆盖后字段重算。
   ⚠️ **已知失效模式——「活跃」是全站口径，会在模型退役期误伤**：某模型只要还有几家中转商没下架、它就一直算活跃，于是**下架最快的、完全正常的**通道反被判成「测不出来」降进备板（2026-09-06 gpt-5.4 实发 7 家，可用性探针还是绿的）。排查入口：备板里 `board_reason_models` 指向一个 relay-pulse 自己**活跃探针已经不用了**的旧模型，就该去看 rpdiag 的 sampler 池是不是没跟上——两边配置不会自动同步。判据、`/api/v1/tasks` 取证配方与「别手工清 latch、2-7 天自愈」见 memory `reference_rpdiag_sampler_pool_lags_retired_model`
...
（v2.89.1 前此处印 `raw_body`，因为当时确实会回退成拿整包信封 grep）。scheduler 与 inline 两条
...
- 总可用率 = `Σ(每块可用率 × 该块探测次数) / Σ探测次数`，即「可用探测数 / 总探测数」
- ⚠️ **不是各时间块等权平均**（2026-09-10 改）：等权会让样本稀疏的块与跑满的块同权，
  典型是 7d 视图里尚未跑满的当天（6 次探测 vs 满块 288 次），实测偏差可达 10pp。
  单一实现在 `frontend/src/utils/monitorDataProcessor.ts` 的 `weightedAvailability`，
  `calculateUptime` / 复合时间块 / 移动端 `heatmapAggregator` / mock 数据全部复用它，别再抄一份。
- **多模型通道（父子层）的可用率 = 该通道所有模型的探测汇总**，不是 `min(各模型可用率)`。
  一个模型故障不再吃掉其余模型的全部成绩（实测 saiai cx O-Pro 4 模型：min 54.92% vs 汇总 66.68%）。
  后端 `internal/automove` 的移板判定同口径（见「非默认约定与硬契约」第 1 条），两边收敛到同一个数。
...
- **native 模板族**（`<service>-native-*`：`cc-native-arith` / `cx-native-arith`）承接第一方厂商的 Anthropic Messages / OpenAI Responses 兼容端点，是**厂商无关**的：刻意不声明 `model`/`request_model`/`model_vendor`，三者必须由监测行按厂商填写。
  ...
  ⚠️ **自 2026-08-21 起本族全部标了 `self_serve_visible: false`，是 admin-only 的**——公开表单已不给提交方任何填模型的入口（见下方「Onboarding 通道标识派生」），一个要求行级填模型的模板留在表单里只会让人选中后卡在「模板未声明模型」。生产上那批火山方舟通道（2026-09-16 起 14 条）继续跑在本族模板上，**变更流程完全不受影响**：`internal/change/index.go` 构建候选与 `defaultTestVariant` 都不看可见性，`buildChangeTestConfig` 走的是 `ResolveVariant` 而非 onboarding 的 `resolveSelfServeTemplate`（守卫 `TestAuthIndex_Rebuild_SelfServeHiddenVariantsStillUsable`，已 bite-test 验非真空）。**要给自助表单加第一方厂商模型，是复制本族模板做一个专属模板，不是把本族标回可见。**
...
- **第一家第一方厂商已接（2026-08-05，v2.73.0）**：火山方舟 `ark`，`channel_type=O` + `channel_source=nat` + `key_type=user`，当日即升 **board=hot**，且全批 `auto_move_exempt`/`auto_cold_exempt` 双真（恒红也不掉板，见下条 ⚠️）。收录时 5 厂商 × cc/cx 共 10 条，**2026-09-16 起 14 条**（GLM/DeepSeek 各补了一条 Flash 低档位）。故生产厂商值现有 8 种（原厂三家 + bytedance/zhipu/moonshot/minimax/deepseek）。
...
**⚠️ cx 侧现在有两套并存的请求形态，新建 cx 模板前先决定照哪套（v2.92.0 起）**：
...
**给自助表单加一个第一方厂商模型 = 建一个专属模板**（2026-08-21 起的唯一做法）：复制该模型对应的 native 模板（cc 侧按思考行为、cx 侧一律 `cx-native-arith`），**body 逐字节保留**，只补 `model` / `request_model` / `model_vendor` / `self_serve_label` 四项。**唯一该重做 body 的情形是厂商事后撤回了对某个请求字段的支持**——那时照新的 native 选型整块换掉、并在 `_comment` 写清证据边界（先例：`cc-glm52-arith` 2026-09-20 由 `-nothink` 改照 `-512`）。

- `model` 写**规范模型 ID**（`glm-5.3`）而不是人话名：它同时是 DB 业务键与热力图展示名，与既有 native 通道写法保持同一个串，将来把那批老行切到专属模板才不会断历史；人话名放 `self_serve_label`。
...
- ⚠️ **模板文件名不跟模型版本走**：模板名是**配置业务键**（生产 `monitors.d`、内联 `config.yaml`、待发布的 onboarding 申请都可能引用它），而配置加载是 fail-closed 的——模板文件找不到会让**整份配置加载失败、容器起不来**（`resolveTemplateForMonitor`）。所以厂商升版本时**改 JSON 里的模型元数据、不改文件名**（请求形态改不改是另一个问题，按兼容性证据定）：`templates/cc-glm52-arith.json` 自 2026-09-20 起跑的是 `glm-5.3`。⚠️ 别被 `self_serve_visible` 误导成「专属模板只活在公开表单里、没人引用、可以随便改名」——`self_serve_visible` 只挡公开表单，admin 发布路径会把 `Submission.TemplateName` 原样写进 monitor 配置。真要改名，只读核对范围是四处：生产 `monitors.d`、内联 `config.yaml` 的 monitors、`onboarding_submissions` 里 pending/approved 的 `template_name`、`change_requests.test_variant`。
- ⚠️ **`_comment` 里要如实区分「实测过的」与「推断的」**。现有 10 个模板的思考行为/reasoning 取舍都源自 2026-08-05 的**火山方舟**实测（`cc-glm52-arith` 的**思考字段选型**是唯一被厂商行为推翻过的一条：2026-08-31 撤回 `thinking.disabled` 支持后，2026-09-20 已按 `cc-native-arith-512` 重做形态），中转商自建网关未逐一验证；`cx-kimi-k27code-arith` 的「`/v1/responses` + 无 `reasoning`」更是个**从未在真端点跑过的组合**（拆自「URL 由端点定、reasoning 认不认由模型定」两条正交推断）。这类不确定性代价有界——自助流程本就是「测通了才准提交」，测不过只是提交不了、不会产生坏数据，走 `/contact` 由我们评估另拆模板。
