...
- **最后同步**: 2026-08-06（生产运行 **v2.76.0**/HEAD=`2dbee1d` + **已部署生产**[prod git_commit=2dbee1d，go1.26.5，health/ready=200，monitors=**284**；回滚锚点 `rollback-20260806-gpt56tl-pre`=部署前 e58e4eb/v2.75.0]）。本轮**接 2026 年年中发布的两批新模型（探针模板 + 一处前端排序，无 schema/无迁移）**，同日连发三版：**v2.74.0**(`7286a04`，锚 `rollback-20260806-gpt56-pre`=ce3c854) / **v2.75.0**(`e58e4eb`，锚 `rollback-20260806-claude5-pre`=7286a04) / **v2.76.0**(`2dbee1d`)。
  ...
  **验证**：`go test ./internal/config ./internal/monitor` + tsc -b + eslint + vitest **291** 全绿；**活体验证走 `InlineProber.ProbeConfig` 真探测内核**——saiai 上 opus-5 200/1632ms、sonnet-5 200/1357ms、fable-5 200/2853ms（同轮 opus-4-8 基线 200/1650ms），gpt-5.6 terra 200/1361ms、luna 200/1283ms（同轮 sol 200/1182ms）；清理后的两个生产模板各取真实通道复验（`cc-opus-ping-20260630`@saiai 200/1534ms、`cc-haiku-titlegen`@bytecatcode 200/4368ms）。**prod 实证**：试点挂在 saiai——`cc/o-max` 的 opus 行**就地换模板**（display `Opus` 不变=历史连续）+ 新增 Sonnet/Fable 子行，`cx/O-web` 新增 GPT-5.6/Terra/Luna 三子行；调度器路径四层全绿（cc：2279/2177/1923ms；cx：GPT 1916、sol 1742、terra 2366、luna 1851ms），monitors 279→**284**。
  ...
  ⚠️ **本轮暴露一个既有 bug（未修，见下方「探针模板」小节与 memory `reference_rp_admin_add_child_model_id_collision`）**：经 admin API 给**已有子行**的通道再追加模板驱动子行时，`monitor_store.mergeChildHiddenFields` 的 Pass 2 按 `parent+model` 展示名兜底匹配，而这类子行的 `model` 来自模板、磁盘上是空串 → 新行全部命中同一既有子行、`copyAdminHiddenFields` 复制其 `model_id` → 校验「model_id 重复」→ **热更新 fail-closed 保住旧配置（监测不断），但磁盘文件已坏、容器重启即加载失败**，且 `/ready` 不报警（其 `config_reload` 只覆盖 `CheckRuntimeModelIDs` 那道闸，本次失败在 `validate()`）。**规避=追加子行时显式写 `model` 展示名**（本轮 terra/luna 即如此，同时也是更稳的做法：`model` 是 DB 业务键）。修法应是把子行匹配改成一对一（既有行被配对即移出候选表）。
...
| GET/HEAD | `/ready` | 就绪检查（含存储连通性；热更新被 `CheckRuntimeModelIDs` fail-closed 闸跳过时 GET body 附 `config_reload{last_skipped_at,last_error,skipped_count}` 信息化，HTTP 状态恒不因此翻 503。⚠️ **热更新有两条静默失败路径，本字段只覆盖其中一条**：另一条是 `loadOrRollback` 里配置加载/`validate()` 失败后「保留旧配置」（如 model_id 重复、yaml 语法错），那条只出 `level=ERROR msg=重载失败` 日志、`/ready` 仍返 `{"status":"ok"}`，**排查热更新不生效必须看日志、别只看 /ready**） |
